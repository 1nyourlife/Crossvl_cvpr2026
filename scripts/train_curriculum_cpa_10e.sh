#!/bin/bash
# Train Florence-2 with Paired Curriculum + CPA (CrossVL full model), 10 epochs.
# Curriculum: epochs 0-3 paired, 4-6 mixed, 7-9 random. CPA weight constant 0.1.
...
CHECKPOINT_DIR="checkpoints/curriculum_cpa_10e_${TIMESTAMP}"
...
cat > "$CHECKPOINT_DIR/training_config.json" <<EOF
{
  ...
  "use_cpa": true,
  "cpa_type": "$CPA_TYPE",
  "cpa_weight_max": $CPA_WEIGHT,
  "use_progressive_cpa_weight": true,
  "curriculum_type": "paired-based",
  "curriculum_schedule": [
    {"epochs": "0-3", "mode": "paired", "cpa_weight": 0.0},
    {"epochs": "4-6", "mode": "mixed",  "cpa_weight": 0.05},
    {"epochs": "7-9", "mode": "random", "cpa_weight": 0.1}
  ],
...
import torch
from transformers import AutoProcessor, AutoModelForCausalLM, TrainingArguments, TrainerCallback
from training.florence_2.florence2_dataset import Florence2Dataset
from training.florence_2.florence2_curriculum_trainer import CustomTrainerWithProgressiveCPA
...
model.cpa = CPA(
    hidden_dim=$CPA_HIDDEN_DIM,
    num_classes=$CPA_NUM_CLASSES,
...
trainer = CustomTrainerWithProgressiveCPA(
    model=model,
    args=training_args,
    train_dataset=None,
    eval_dataset=None,
